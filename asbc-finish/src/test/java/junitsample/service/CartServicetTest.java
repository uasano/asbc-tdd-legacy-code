...
public class CartServicetTest {
	...
	@Test
	public void testカート明細が０件のときにカート明細存在チェックがfalseを返すべき() {
		int userId = 1;
		...
		assertFalse(sut.existCartItems(userId));
	}
	@Test
	public void testカート明細が1件のときにカート明細存在チェックがtrueを返すべき() {
		int userId = 1;
		...
		assertTrue(sut.existCartItems(userId));
	}
